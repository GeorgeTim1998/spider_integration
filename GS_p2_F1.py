...
psi_level = 25e-6 # used to calc contours with desired level
Fpl_vs_Fvac_ratio = lao_hash['Fpl_vs_Fvac_ratio']

sup.create_readmi(lao_hash, my_dir, p_pow=2, F_pow=1)
...
#%% Define variational problem
  ...
  a = dot(grad(u)/r, grad(r2*v))*dx - M0 * (2*p0 * u/psi0**2)*r2 * r*v*dx
  f = Fpl_vs_Fvac_ratio*Constant(0.5 * F2_0/psi0)
  L = f * r*v*dx
...
#%% Compute solution and p(psi), F(psi), J(psi)
  ...
  p_psi = fsup.calculate_p_psi_pow_2(psi0, u, p0)
  F2_psi = fsup.calculate_F2_pow_1(F2_0, psi0, u, Fpl_vs_Fvac_ratio)
  [J_psi, I] = fsup.calculate_J_psi_p_pow_2_F_pow_1(p0, psi0, F2_0, r, V, dx, u, Fpl_vs_Fvac_ratio)
  fsup.print_colored('Ipl =', 'blue', I)
